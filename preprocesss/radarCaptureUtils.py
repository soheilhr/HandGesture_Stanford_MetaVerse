...
#                print(packet_num)
    ...
    def decode_data(data_buffer,num_Rx=1): 
    ...
    #    chirp_block=np.concatenate([block[1:] for block in blocks if len(block)==num_chirps+1],0)    
        chirp_block=([block[1:] for block in blocks if len(block)==num_chirps+1])

        frames=[]
        for frame in chirp_block:
            if (np.var([len(block) for block in frame])==0):
                frames.append(frame)
        ...
        dat=chirp_data.reshape((-1,num_chirps,num_Rx,samples_per_chirp))
        headers={'frame_header':frame_header,
                 'chirp header':chirp_header
        }

        ######################### 
        return dat, p_list, headers
